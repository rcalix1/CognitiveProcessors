...
void loop()
{
    ...
    // wait 10 minute and then re-run the test 
    delay(1000 * 60 * 10);
}
...
int RicardoTest()
{
  ...
  int result = 0;
    ...
    unsigned short nids[] = { 0x0000, 0x0000 };  // neur ids
   ////////////////////////////////////////////////////////////////////////////////////////////////////
  initialize_SD_card();
  myFile = SD.open("TR4096.TXT"); //TEST9.TXT
   int num_cols = FeaturesGlobal;
    int the_class;
    int sample_n = 0;
  ...
  if (myFile) {
    ...
    while (myFile.available()) {
        ...
        if (aurora_count > num_cols)
        {
          ...
          //printf("\nRecognize non equi-distant vector ...\n"); // 12,12,12,12
      
      //current_vector = { 70, 48, 31,  5 };  //remove ricardo test
      if ( EN_KNN == 1 ){ cognimem_write(CM_REG_NETWORK_STATUS, 32);}
      result = cognimem_recognize(current_vector, sizeof(current_vector));
      sample_n = sample_n + 1;
    
      //printf("Status: (identified, uncertain, or unknown): %02X\n", result);
      //printf("actual class: %02u\n", the_class);
      for (int i = 0; i < k; i++)
      {
